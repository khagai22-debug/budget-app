...
.hero-box {
    ...
    border-radius: 24px;
    padding: 24px;
    box-shadow: 0 14px 40px rgba(19,78,74,0.25);
    margin-bottom: 1rem;
}
.section-box {
    background: white;
...
try:
    ...
    total_spent = float(df_tx["סכום"].sum()) if not df_tx.empty and "סכום" in df_tx.columns else 0.0
    ...
    with left:
        ...
        with st.form("add_tx_form", clear_on_submit=True):
            ...
            business = st.selectbox(
                "שם בית העסק (אם קיים במילון)",
                options=[""] + business_names,
                index=0,
                help="אפשר לבחור עסק קיים כדי להימנע מהקלדה"
            )
    ...
    with right:
        ...
        if cat_summary.empty:
            ...
            st.bar_chart(cat_summary.set_index("קטגוריה_לתצוגה"))
        ...
        if cat_summary.empty:
            ...
            pie_df = cat_summary.rename(columns={"קטגוריה_לתצוגה": "קטגוריה", "סכום": "סכום"})
            st.plotly_chart(
                {
                    "data": [{
                        "labels": pie_df["קטגוריה"],
                        "values": pie_df["סכום"],
                        "type": "pie",
                        "hole": 0.45,
