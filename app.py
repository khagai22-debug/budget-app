...
try:
    ...
    # הוספנו את הטאב השלישי!
    tab1, tab2, tab3 = st.tabs(["📊 דשבורד נוכחי (גוגל שיטס)", "📥 ניתוח אקסל מהבנק", "⚙️ ניהול ושיוך חכם"])
    ...
    # הטאב החדש לניהול חכם של עסקאות שלא שויכו
    with tab3:
        st.markdown('<div class="section-box">', unsafe_allow_html=True)
        st.markdown("<h3 style='color: #1e293b; margin-bottom: 10px;'>⚙️ שיוך חכם למילון העסקים</h3>", unsafe_allow_html=True)
        st.write("כאן תוכל ללמד את המערכת לאיזו קטגוריה שייכים בתי עסק שמופיעים כ־'לא משויך'. ברגע שתשייך בית עסק, הוא יסווג אוטומטית מעתה והלאה!")

        unassigned_df = df_tx[df_tx["קטגוריה_לתצוגה"] == "לא משויך"]
        unassigned_list = unassigned_df["שם עסק באשראי"].dropna().unique().tolist()
        unassigned_list = [str(x).strip() for x in unassigned_list if str(x).strip()]

        c1, c2 = st.columns([1, 1.2])
        with c1:
            st.markdown("<h4 style='color: #3b82f6; margin-bottom: 20px;'>🎯 הוספת שיוך חדש</h4>", unsafe_allow_html=True)
            with st.form("dictionary_form", clear_on_submit=True):
                selected_biz = st.selectbox("בחר עסק מתוך המאגר שלא שויך", [""] + unassigned_list)
                manual_biz = st.text_input("או הקלד ידנית עסק חדש (למשל מהאקסל שהעלית)")
                
                valid_cats = [c for c in CATEGORIES if c not in ["עסק מוכר - סווג אוטומטית לפי מילון", "לא משויך"]]
                selected_cat = st.selectbox("בחר את הקטגוריה המתאימה", valid_cats)
                
                st.markdown("<br>", unsafe_allow_html=True)
                submit_dict = st.form_submit_button("💾 עדכן את מילון העסקים", type="primary")
                
                if submit_dict:
                    final_biz = manual_biz.strip() if manual_biz.strip() else selected_biz.strip()
                    if final_biz:
                        try:
                            client = get_gspread_client()
                            wb = client.open_by_url(SPREADSHEET_URL)
                            dict_sheet = wb.worksheet("מילון_עסקים")
                            dict_sheet.append_row([final_biz, selected_cat])
                            load_data.clear()
                            st.success(f"✅ העסק '{final_biz}' שויך בהצלחה לקטגוריה '{selected_cat}'!")
                            st.rerun() # מרענן את העמוד כדי שהעסק ייעלם מרשימת הלא משויכים
                        except Exception as e:
                            st.error(f"שגיאה בשמירת הנתונים: {e}")
                    else:
                        st.error("נא לבחור מהרשימה או להקליד שם בית עסק.")
        
        with c2:
            st.markdown("<h4 style='color: #3b82f6; margin-bottom: 20px;'>🔍 עסקאות ללא שיוך במאגר</h4>", unsafe_allow_html=True)
            if unassigned_df.empty:
                st.success("מעולה! כל העסקאות הקיימות במאגר משויכות לקטגוריות מתאימות.")
            else:
                show_unassigned = unassigned_df[["תאריך", "שם עסק באשראי", "סכום"]].copy()
                show_unassigned["סכום"] = show_unassigned["סכום"].map(lambda x: f"₪{x:,.0f}")
                st.dataframe(show_unassigned, use_container_width=True, hide_index=True)
