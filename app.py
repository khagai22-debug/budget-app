...
def process_uploaded_excel(uploaded_file, df_dict):
    try:
        # Load excel. Based on the file structure, headers are at row 3 (index 3)
        ...
        # Auto classification based on dictionary
        dict_mapping = {}
...
try:
    ...
    tab1, tab2 = st.tabs(["📊 דשבורד נוכחי (מתוך גוגל שיטס)", "📥 העלאת פירוט אשראי (סימולציה)"])
    ...
    with tab2:
        st.markdown('<div class="section-box">', unsafe_allow_html=True)
        st.subheader("📥 העלאת פירוט חיובים מאשראי")
        st.write("העלה לכאן קובץ אקסל שהורדת מחברת האשראי, ותוכל לראות מיד סיכום והתפלגות שלו, בלי לשמור או לשנות שום דבר ב־Google Sheets.")
